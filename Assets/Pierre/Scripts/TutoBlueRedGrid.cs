...
public class TutoBlueRedGrid : MonoBehaviour
{
    ...
    private static TutoBlueRedGrid _instance = null;

    public static TutoBlueRedGrid Instance
    {
        get => _instance;
    }
    private void Awake()
    ...
    public void movementsPossible(int xStart, int yStart, bool enemy)
    {
        ...
        Panel startPanel = TutoGrid.Instance.gridArray[xStart, -yStart];
    ...
    private List<Panel> CheckVoisin(Panel currentPanel)
    {
        ...
        {
            voisinList.Add(TutoGrid.Instance.gridArray[currentPanel.x - 1, currentPanel.y]);
        }

        if (currentPanel.x + 1 < TutoGrid.Instance.width) // Right
        {
            voisinList.Add(TutoGrid.Instance.gridArray[currentPanel.x + 1, currentPanel.y]);
        }
        ...
        {
            voisinList.Add(TutoGrid.Instance.gridArray[currentPanel.x, currentPanel.y - 1]);
        }

        if (currentPanel.y + 1 < TutoGrid.Instance.height) // Up
        {
            voisinList.Add(TutoGrid.Instance.gridArray[currentPanel.x, currentPanel.y + 1]);
        }
